...
SPAM_PHRASES = [
    ...
    # Убраны упоминания опасностей (по требованию)
...
# Важные ключевые слова, которые должны ПРОПУСКАТЬСЯ (без упоминаний опасностей)
IMPORTANT_KEYWORDS = [
...
SPAM_URL_THRESHOLD = 3  # Увеличили порог ссылок
...
async def send_news_to_user(bot_client, user_id, posts):
    ...
    try:
        ...
        await bot_client.send_message(
            ...
            f"🛡️ Фильтры: спам, дубликаты, скам, опасности\n"
            f"────────────────",
            parse_mode='md',
            link_preview=False
        )
...
async def main():
    ...
    async def start_handler(event):
        ...
        await event.reply(
            ...
            "🛑 *Фильтр опасностей:* включен (не показываем ракетные/авиационные тревоги)\n\n"
            "✨ Команды:\n"
